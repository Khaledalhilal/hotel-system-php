...
$sql = "SELECT reservation.*, rooms.*, users.* FROM reservation INNER JOIN rooms ON reservation.room_id = rooms.room_id INNER JOIN users ON reservation.user_id = users.user_id where users.user_id='$user_id'";
                        ...
                        <div class=" row mt-4">
                            <div class="table-responsive">
                                <table class="table display data-table text-nowrap table-striped" id="dataTable" role="grid">
                                    <thead>
                                        <tr role="row">
                                            <th class="sorting text-center text-white" style="background-color: #1cc3b2;">numberOfChildrens</th>
                                            <th class="sorting text-center text-white" style="background-color: #1cc3b2;">gender Type</th>
                                    ...
                                    <tbody>
                                        <?php foreach ($reservations as $res) { ?>
                                            <tr class="text-center">
                                                <td id=""><?php echo $res['numberOfChildrens']  ?></td>
                                                <td id=""><?php echo  $res['gender'] ?></td>
